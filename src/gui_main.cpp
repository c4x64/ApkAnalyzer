...
void OpenApkDialog(HWND hwnd) {
    ...
    if (GetOpenFileNameA(&ofn)) {
        ...
        Menu::clearData(); // Clear previous session
        ...
        for (const auto& file : files) {
            if (file.find(".so") != std::string::npos) {
                ...
                if (parser.isValid()) Menu::getSymbolsMap()[file] = parser.getSymbols();
            }
            if (file.find(".dex") != std::string::npos) {
                auto data = ZipUtility::readFile(apkPath, file);
                DexParser parser(data);
                if (parser.isValid()) {
                    Menu::getCurrentDexData() = data;
                    Menu::setMethods(parser.getMethods());
                    Menu::setFields(parser.getFields());
                    Menu::setClasses(parser.getClasses());
                }
            }
        }
    }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowExW(0, wc.lpszClassName, L"APK Offset Generator - Windows GUI", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);

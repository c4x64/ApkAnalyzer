...
bool MemoryUtils::isMemoryReadable(uintptr_t address, size_t size) {
    ...
    if (VirtualQuery((LPCVOID)address, &mbi, sizeof(mbi))) {
        return (mbi.Protect & PAGE_READONLY || mbi.Protect & PAGE_READWRITE || mbi.Protect & PAGE_EXECUTE_READ || mbi.Protect & PAGE_EXECUTE_READWRITE);
    }
    return false;
#else
    // Using msync to check if the memory is mapped and readable
    return msync((void*)(address & ~(sysconf(_SC_PAGESIZE) - 1)), size, MS_ASYNC) == 0;
#endif
}
